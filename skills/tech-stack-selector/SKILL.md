...
description: 技术栈选型专家。基于一个经过验证的、分类清晰的技术栈目录，为不同类型和阶段的项目（全栈Web、移动端、前端SPA、后端API等）提供快速、匹配的技术方案推荐、架构蓝图和关键决策逻辑。
---
...
| **跨平台移动端 App**                                       | **React Native 方案** | -            | **决策点**：是否需要使用React技术栈开发iOS和Android应用。<br>**优势**：使用Ignite获得高质量脚手架，配合Tamagui实现高性能、可定制UI，共享大部分业务逻辑。                              |
...
| **后端 API 服务** <br> _(团队熟悉JS/TS)_                   | **Node 后端方案**     | Python 方案  | **决策点**：团队是否熟悉Node.js/TypeScript生态。<br>**优势**：HonoJS轻量且适用于边缘运行时，Prisma提供卓越的类型安全和开发体验，BullMQ处理异步任务，全栈可统一使用TypeScript。        |

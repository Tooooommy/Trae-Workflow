---
name: i18n-patterns
description: 国际化模式 - 多语言、本地化、区域设置最佳实践。**必须激活当**：用户要求实现国际化、支持多语言或处理本地化时。即使用户没有明确说"i18n"，当涉及多语言支持、区域设置或本地化时也应使用。
---
...
```mermaid
flowchart LR
    subgraph Detection["语言检测"]
        BD["浏览器设置"]
        URL["URL 参数"]
        CK["Cookie"]
    end

    subgraph Loading["翻译加载"]
        TF["翻译文件"]
        JSON["JSON/YAML"]
        DB["数据库"]
    end

    subgraph Formatting["格式化"]
        FMT["日期/数字"]
        CUR["货币"]
        PL["复数"]
        SORT["排序规则"]

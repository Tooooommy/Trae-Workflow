...
description: 统一错误处理模式 - 结构化错误响应与异常管理最佳实践。**必须激活当**：用户要求实现错误处理、设计异常管理或构建统一错误响应时。即使用户没有明确说"错误处理"，当涉及异常捕获、错误码设计或日志记录时也应使用。
---
...
```typescript
function tryCatch<T, E = Error>(fn: () => T): [T | null, E | null] {
  try {
    return [fn(), null];
  } catch (error) {

...
```mermaid
flowchart TB
    subgraph Frameworks["框架与驱动层 (Frameworks & Drivers)"]
        FW["Web框架、数据库、外部服务"]
    end

    subgraph Interfaces["接口适配层 (Interface Adapters)"]
        IA["控制器、网关、转换器"]
    end
    ...
    subgraph Entities["实体层 (Entities)"]
        EN["业务核心规则和数据结构"]
    end

    Frameworks --> Interfaces --> UseCases --> Entities
```

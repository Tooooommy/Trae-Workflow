...
description: 整洁架构 - 分层设计、依赖倒置、可测试性最佳实践
---
...
namespace UseCases {
  ...
  export class CreateUser implements CreateUserUseCase {
    ...
    async execute(input: CreateUserInput): Promise<CreateUserOutput> {
      ...
      const user = new Domain.UserEntity(
        crypto.randomUUID(),
        input.email,
        input.name
      );
...
class CreateUserUseCaseImpl implements UseCases.CreateUserUseCase {
  constructor(
    @inject(TYPES.UserRepository) private userRepository: Domain.UserRepository
  ) {}
...
container.bind<Domain.UserRepository>(TYPES.UserRepository)
  .to(Infrastructure.PostgresUserRepository);

container.bind<UseCases.CreateUserUseCase>(TYPES.CreateUserUseCase)

...
Aerich 是 Tortoise ORM 的数据库迁移工具，常用于 FastAPI 等异步 Python 项目。

### 工作流

```bash
# 初始化Aerich（在Tortoise ORM配置完成后运行一次）
aerich init -t your_project.module.TORTOISE_ORM_CONFIG_VARIABLE
aerich init-db  # 生成初始迁移（如果起始版本不是0）

# 1. 修改你的Tortoise ORM模型（通常在 models.py 中）

# 2. 生成迁移文件
aerich migrate --name add_user_avatar  # 自动对比模型与数据库差异生成迁移
...
class User(models.Model):
    ...
    avatar_url = fields.CharField(max_length=255, null=True) # <- 新增的字段
...
对于 Aerich 无法自动生成的复杂操作（如并发索引、高级数据回填），可以编写纯 SQL 迁移文件：

```bash
# 生成一个空的迁移文件模板
aerich migrate --empty --name add_concurrent_index
```

```sql
-- 注意：CONCURRENTLY 不能在事务块内运行，需要在迁移工具中特殊处理或确保此迁移不在事务中执行。
-- 安全操作：在低峰期手动执行，或通过Aerich运行但需确认其事务行为。

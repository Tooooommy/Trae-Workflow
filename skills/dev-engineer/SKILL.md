...
**优先由 project-manager 调度激活**（阶段3：并行开发 - 内建质量）
...
| 类型 | 来源/输出        | 文档        | 路径                                                                                        | 说明             |
| ---- | ---------------- | ----------- | ------------------------------------------------------------------------------------------- | ---------------- |
| 输入 | product-designer | PRD、设计稿 | `docs/01-requirements/{project-name}-prd.md`<br>`docs/02-design/ui-design-*.md`             | 功能需求、UI设计 |
...
flowchart TD
    ...
    G --> G1[quality-engineer<br/>功能测试]
    G --> G2[security-auditor<br/>安全扫描]
    G1 -->|Bug| H[修复Bug] --> D
    G2 -->|漏洞| I[修复漏洞] --> D
    G1 & G2 -->|通过| J[输出代码]
    J --> K[传递任务]
...
7. **并行质量检查**
   - 同步提交给 quality-engineer 进行功能测试
   - 同步提交给 security-auditor 进行安全扫描
   - 发现问题即时修复

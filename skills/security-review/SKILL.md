...
description: 在添加身份验证、处理用户输入、处理机密信息、创建API端点或实现支付/敏感功能时使用此技能。提供全面的安全检查清单和模式。**必须激活当**：用户要求实现身份验证、处理用户输入、管理密钥、创建 API、处理支付或存储敏感数据时。即使用户没有明确说"安全"，当涉及安全相关任务时也应使用。
---

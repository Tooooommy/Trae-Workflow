---
name: ddd-patterns
description: 领域驱动设计 - 限界上下文、聚合、值对象最佳实践。**必须激活当**：用户要求设计领域模型、实现 DDD 模式或构建复杂业务逻辑时。即使用户没有明确说"DDD"，当涉及限界上下文、聚合根或值对象设计时也应使用。
---
...
flowchart TB
    ...
    Order --- |上下文映射| Inventory
    Inventory --- |上下文映射| Payment
```

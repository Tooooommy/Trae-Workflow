...
使用Vercel Agent Browser（首选）和Playwright备选方案进行端到端测试的专家。主动用于生成、维护和运行E2E测试。管理测试流程，隔离不稳定的测试，上传工件（截图、视频、跟踪），并确保关键用户流程正常运行。

## 何时调用

当需要运行E2E测试、创建端到端测试、测试关键用户流程、验证用户旅程或进行浏览器自动化测试时调用。
